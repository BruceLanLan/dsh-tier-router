...
const tierSchema = z.object({
  ...
  cheapProvider: z.string().default('cctq'),
  cheapModel: z.string().default('gpt-5.6-terra'),
...
export function apply(ctx) {
  ...
  function activeFallbackTarget(agent, tierName) {
    ...
    if (rec.index < 0 || rec.until <= now || rec.index >= chain.length) {
  ...
  function triggerFallback(agent, tierName, failureText) {
    ...
    const now = Date.now()
    if (!rec || rec.index < 0 || rec.until <= now) rec = { index: -1, until: 0 }
  ...
  async function applyRoute(tier, persist, agent) {
    if (tier === 'auto') {
      ...
      if (agent && !isChildAgent(agent)) ensureHeader(agent, effectiveTarget(agent, planActive(agent) ? 'strong' : 'cheap'))
    ...
    if (tier === 'delegated') {
      ...
      if (agent && !isChildAgent(agent)) {
        const selected = defaultSelectionTarget()
        if (selected) ensureHeader(agent, selected)
      }
    ...
    clearRoutingTransients(agent)
    if (agent && !isChildAgent(agent)) ensureHeader(agent, effectiveTarget(agent, tier))
    let persisted = ''
  ...
  async function applyConfigure(args, agent) {
    ...
    const next = migrateTierConfig(tierName, {
      ...
      fallback: Array.isArray(args.fallback) && args.fallback.length > 0 ? args.fallback : current.fallback,
    })
    ...
    if (agent && !isChildAgent(agent) && (effectiveMode(agent) === tierName || effectiveMode(agent) === 'auto')) ensureHeader(agent, effectiveTarget(agent, tierName))
    let persisted = ''
  ...
  if (typeof ctx.on === 'function') {
    ctx.on('agent/inbox/inserted', (payload) => {
      ...
      try {
        ...
        // Fallback recovery is conservative by design: the rc.7 agent loop emits
        // no LLM success event, so a fallback stays active until its TTL expires.
        // resolveTarget() keeps writing the fallback header while TTL is valid and
        // falls back to the primary once expired (or after a mode switch).
    ...
    ctx.on('agent/error', async (payload) => {
      ...
      try {
        ...
        if (escalatedNow) {
          ...
          if (!isChildAgent(agent)) ensureHeader(agent, effectiveTarget(agent, 'strong'))
          else {
            try { childTiers.set(agent, 'strong') } catch (e) {}
          }
        }
        ...
        if (cls === 'fallback') {
          ...
          if (tierName) {
            ...
            if (outcome && outcome.target) ensureHeader(agent, outcome.target)
          }
        }
        if (cls !== 'ignore' && !escalatedNow && tierNameFor(agent) === 'cheap') {
  ...
  if (tools) {
        ...
        try {
          if (exec.agent && fallbackStates.has(exec.agent)) {
            ...
            if (fallbackActive(rec, Date.now())) {
              const tname = tierNameFor(exec.agent)
              const chain = tname ? chainFor(exec.agent, tname) : []
              fallbackLine = '  fallback state: index=' + rec.index + '/' + chain.length + ' until=' + new Date(rec.until).toISOString() + ' (target ' + (chain[rec.index] ? chain[rec.index].provider + '/' + chain[rec.index].model : '?') + ')'
            }
          }
        } catch (e) {}
        const lines = [
    ...
    const configureTool = defineTool({
      name: 'tier_configure',
      description: 'Configure which provider/model/effort backs each tier (strong and cheap), optional followSession mode and fallback chains, and the subagent policy. Any registered provider and model id work. Efforts are validated against the target model\'s declared reasoning efforts when metadata is available (e.g. cctq models declare low/medium/high). The configuration persists in the tier-router settings namespace (pass sessionOnly: true for a transient change); persist: true also writes the session default model.',
      parameters: {
        tier: { type: 'string', required: true, enum: ['strong', 'cheap'], description: 'Which tier to configure.' },
        provider: { type: 'string', description: 'Registered provider route, e.g. "cctq". Default keeps the current value.' },
        model: { type: 'string', description: 'Model id for that provider, e.g. "gpt-5.6-terra". Default keeps the current value.' },
      ...
      handler: async (invocation) => {
        ...
        try {
          ...
          if (sub === 'plan') {
            ...
            ensureHeader(invocation.agent, effectiveTarget(invocation.agent, 'strong'))
