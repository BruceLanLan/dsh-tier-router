...
export function apply(ctx) {
  ...
  function statusText() {
    ...
    return [
      ...
      '  mode: ' + state.mode + ' (escalate: ' + state.escalateThreshold + ' errors / ' + Math.round(state.escalateWindowMs / 1000) + 's window -> ' + Math.round(state.escalateTtlMs / 1000) + 's strong)',
  ...
  async function applyRoute(tier, persist, agent) {
    if (tier === 'auto' || tier === 'off') {
      state.mode = tier
      if (agent && !isChildAgent(agent)) {
        if (tier === 'auto') ensureHeader(agent, planActive(agent) ? state.strong : state.cheap)
        else {
          const def = defaultSelectionTarget()
          if (def) ensureHeader(agent, def)
        }
      }
      return { applied: tier, message: 'Routing mode set to "' + tier + '". ' + (tier === 'auto' ? 'Plan-mode steps run on the strong tier; execution steps on the cheap tier.' : 'Per-step routing disabled; session returned to its default model; only explicit commands and tools act.') }
    }
    ...
    return { applied: tier, message: 'Steps now route to the ' + tier + ' tier (' + tierLabel(target) + ').' + persisted }
  }
  ...
  async function applyConfigure(args, agent) {
    ...
    if (agent && !isChildAgent(agent) && (state.mode === tierName || state.mode === 'auto')) ensureHeader(agent, state[tierName])
    let persisted = ''
  ...
  if (typeof ctx.on === 'function') {
    ctx.on('agent/request', async (payload, next) => {
      ...
      try {
        if (state.mode === 'off') { diag.lastRouting = 'step' + stepNo + ': mode=off, untouched (' + agentIdentity(payload && payload.agent) + ')'; return config }
  ...
  if (tools) {
    ...
    const routeTool = defineTool({
      name: 'tier_route',
      description: 'Switch which tier handles this session. strong = ' + tierLabel(state.strong) + ' for hard stretches (architecture, debugging, design); cheap = ' + tierLabel(state.cheap) + ' for routine implementation; auto = strong while plan mode is active and cheap while executing; off = disable per-step routing and return the session to its default model.',
      parameters: {
  ...
  if (commands) {
    commands.register({
      ...
      handler: async (invocation) => {
        ...
        if (!q) return { kind: 'success', text: statusText() }
        try {
    ...
    commands.register({
      ...
      handler: async (invocation) => {
        ...
        const arg = rest.join(' ')
        try {
          if (sub === 'status') return { kind: 'success', text: statusText() }
  ...
  if (systemPrompt) {
    systemPrompt.section({
      ...
      text: function () {
        return [
          '## Tiered model routing',
          'This session uses tiered routing: strong = ' + tierLabel(state.strong) + '; cheap = ' + tierLabel(state.cheap) + '. Routing mode: ' + state.mode + '; subagent policy: ' + state.subagentPolicy + '.',
