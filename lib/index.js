...
// the agent/request waterfall. Tier configuration persists in the
...
export function apply(ctx) {
  ...
  if (settings && typeof settings.register === 'function') {
    try {
      ...
      state.strong = { provider: saved.strongProvider, model: saved.strongModel, reasoningEffort: saved.strongEffort, label: 'strong' }
      state.cheap = { provider: saved.cheapProvider, model: saved.cheapModel, reasoningEffort: saved.cheapEffort, label: 'cheap' }
      state.subagentPolicy = saved.subagentPolicy
      settingsScope.watch((next) => {
        try {
          state.strong = { provider: next.strongProvider, model: next.strongModel, reasoningEffort: next.strongEffort, label: 'strong' }
          state.cheap = { provider: next.cheapProvider, model: next.cheapModel, reasoningEffort: next.cheapEffort, label: 'cheap' }
      ...
      try {
        ...
        if (rec.count >= state.escalateThreshold && rec.until < now) {
          ...
          if (!isChildAgent(agent)) ensureHeader(agent, state.strong)
            ...
            // Escalate ANY child (tier_worker children and built-in-subagent
            // children alike): the explicit tier overrides policy and mode.
  ...
  if (tools) {
    ...
    const configureTool = defineTool({
      name: 'tier_configure',
      description: 'Configure which provider/model/effort backs each tier (strong and cheap) and the subagent policy. Any registered provider and model id work, e.g. deepseek-official/deepseek-v4-pro or other mounted routes. The configuration persists in the tier-router settings namespace (pass sessionOnly: true for a transient change); persist: true also writes the session default model.',
      parameters: {
      ...
      handler: async (invocation) => {
        ...
        try {
          ...
          if (sub === 'set') {
            ...
            const r = await applyConfigure({ tier: tier, provider: provider, model: model, reasoningEffort: effort, sessionOnly: false, persist: false }, invocation.agent)
  ...
  console.log('tier: active — mode=' + state.mode + ' strong=' + tierLabel(state.strong) + ' cheap=' + tierLabel(state.cheap) + ' subagentPolicy=' + state.subagentPolicy + ' guard=on escalate=on persist=' + (settingsScope ? 'settings' : 'memory'))
}
