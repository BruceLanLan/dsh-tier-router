 ...
 * selection, cheap starts at medium on cctq/gpt-5.6-terra; both ship a
 * deepseek-official fallback so a provider outage never strands a task.
 */
...
export function migrateTierConfig(tierName, raw) {
  ...
  let fallback = []
  if (Array.isArray(raw.fallback) && raw.fallback.length > 0) {
